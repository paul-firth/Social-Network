...
@section('content')
    ...
    @if (count($errors) > 0)
        <div class="alert">
            <ul>
                @foreach ($errors->all() as $error)  <!-- erroe cheacking  --> 
                    <li>{{$error}}</li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="panel panel-default">
             <div class="panel-heading">Register</div>
                <div class="panel-body">
                    <form method="POST" action="/user" enctype="multipart/form-data">       <!-- Submits form to UserControler path not RegisterController as built registration into user controller  --> 
                        {{csrf_field()}}
